...
async function verifyPage(filePath) {
  ...
  try {
    ...
    if (coverLinks.length !== 2) {
      throw new Error("The author and voice links are missing.");
    }
    if (coverLinks[0].href !== "https://www.instagram.com/marco.buttu/") {
      throw new Error("The author link is incorrect.");
    }
    if (coverLinks[1].href !== "https://www.ashtangasaadhana.com/") {
...
async function main() {
  ...
  await verifyPage(path.join(root, "dist/sutra_by_heart_embedded.html"));
